...
export async function insertJSON<T extends { id?: string }>(table: string, id: string, obj: T): Promise<void> {
  const db = getDb();
  await db.execute({ sql: `INSERT INTO ${table} (id,data) VALUES (?, ?)`, args: [id, JSON.stringify(obj)] });
}

export async function updateJSON<T extends { id?: string }>(table: string, id: string, obj: T): Promise<number> {
  const db = getDb();
