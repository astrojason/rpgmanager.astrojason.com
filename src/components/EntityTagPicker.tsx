...
export default function EntityTagPicker({
  ...
  function renderList(
    ...
    if (items.length === 0) {
      return <div style={{ padding: "12px 14px", color: "var(--grim-ink-4)", fontFamily: "var(--font-body)", fontSize: "1.4444rem" }}>{emptyMsg}</div>;
    }
    return items.map(item => (
      <label key={item.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "6px 14px", cursor: "pointer", background: selected.includes(item.id) ? `oklch(0.72 0.165 48 / 0.12)` : "transparent" }}>
        ...
        <span style={{ fontFamily: "var(--font-body)", fontSize: "1.4444rem", color: "var(--grim-ink-2)" }}>{item.name}</span>
      </label>
    ));
  }
  ...
  return (
    <div>
      <div className="grim-label" style={{ marginBottom: 8 }}>
        ...
        {totalSelected > 0 && (
          <span className="grim-chip is-ember" style={{ marginLeft: 8, fontSize: "1.1111rem", padding: "1px 7px" }}>
            {totalSelected}
          </span>
        )}
      </div>
      ...
      {totalSelected > 0 && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 10 }}>
          {selectedNpcs.map(id => {
            ...
            return n ? (
              <span key={id} className="grim-chip is-ember" style={{ fontSize: "1.2222rem", cursor: "pointer", display: "flex", alignItems: "center", gap: 5 }} onClick={() => toggle(id, selectedNpcs, onNpcsChange)}>
                {n.name} ×
              </span>
            ) : null;
          })}
          {selectedLocations.map(id => {
            ...
            return l ? (
              <span key={id} className="grim-chip is-arcane" style={{ fontSize: "1.2222rem", cursor: "pointer", display: "flex", alignItems: "center", gap: 5 }} onClick={() => toggle(id, selectedLocations, onLocationsChange ?? (() => {}))}>
                {l.name} ×
              </span>
            ) : null;
          })}
          {selectedPcs.map(id => {
            ...
            return p ? (
              <span key={id} className="grim-chip" style={{ fontSize: "1.2222rem", cursor: "pointer", display: "flex", alignItems: "center", gap: 5, background: "oklch(0.55 0.090 145 / 0.18)", border: "1px solid oklch(0.55 0.090 145 / 0.45)", color: "var(--grim-moss)" }} onClick={() => toggle(id, selectedPcs, onPcsChange ?? (() => {}))}>
                {p.name} ×
              </span>
            ) : null;
          })}
          {selectedQuests.map(id => {
            ...
            return qt ? (
              <span key={id} className="grim-chip is-faction" style={{ fontSize: "1.2222rem", cursor: "pointer", display: "flex", alignItems: "center", gap: 5 }} onClick={() => toggle(id, selectedQuests, onQuestsChange ?? (() => {}))}>
                {qt.name} ×
              </span>
            ) : null;
          })}
          {selectedItems.map(id => {
            ...
            return it ? (
              <span key={id} className="grim-chip" style={{ fontSize: "1.2222rem", cursor: "pointer", display: "flex", alignItems: "center", gap: 5, background: "oklch(0.55 0.090 145 / 0.18)", border: "1px solid oklch(0.55 0.090 145 / 0.45)", color: "var(--grim-moss)" }} onClick={() => toggle(id, selectedItems, onItemsChange ?? (() => {}))}>
                ⚔ {it.name} ×
              </span>
            ) : null;
          })}
          {selectedFactions.map(id => {
            ...
            return f ? (
              <span key={id} className="grim-chip" style={{ fontSize: "1.2222rem", cursor: "pointer", display: "flex", alignItems: "center", gap: 5, background: "oklch(0.50 0.14 285 / 0.18)", border: "1px solid oklch(0.50 0.14 285 / 0.45)", color: "var(--grim-arcane)" }} onClick={() => toggle(id, selectedFactions, onFactionsChange ?? (() => {}))}>
                ⚑ {f.name} ×
              </span>
            ) : null;
          })}
          {selectedDeities.map(id => {
            ...
            return d ? (
              <span key={id} className="grim-chip" style={{ fontSize: "1.2222rem", cursor: "pointer", display: "flex", alignItems: "center", gap: 5, background: "oklch(0.55 0.10 60 / 0.18)", border: "1px solid oklch(0.55 0.10 60 / 0.45)", color: "var(--grim-gold)" }} onClick={() => toggle(id, selectedDeities, onDeitiesChange ?? (() => {}))}>
                ✦ {d.name} ×
              </span>
            ) : null;
          })}
        ...
        <div style={{ maxHeight: 180, overflowY: "auto", padding: "6px 0" }}>
          {activeTab === "npcs" ? (
            filteredNpcs.length === 0
              ? <div style={{ padding: "12px 14px", color: "var(--grim-ink-4)", fontFamily: "var(--font-body)", fontSize: "1.4444rem" }}>No NPCs found</div>
              : filteredNpcs.map(n => (
            ...
            filteredLocations.length === 0
              ? <div style={{ padding: "12px 14px", color: "var(--grim-ink-4)", fontFamily: "var(--font-body)", fontSize: "1.4444rem" }}>No locations found</div>
