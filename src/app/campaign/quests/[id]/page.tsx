    ...
    return (
      <div style={{ padding: "36px 56px" }}>
        ...
        <div style={{ textAlign: "center", padding: "60px 24px", color: "var(--grim-ink-4)" }}>
          <div style={{ fontFamily: "var(--font-display)", fontSize: "4rem", color: "var(--grim-ink-3)", marginBottom: 8 }}>~ errand not found ~</div>
        </div>
      </div>
    );
  }
      ...
      <section className="grim-tome is-bordered" style={{ padding: 0, overflow: "hidden", display: "flex" }}>
        ...
        <div style={{ flex: 1, padding: "28px 32px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16 }}>
            <div>
              <h1 style={{
                fontFamily: "var(--font-display)", fontSize: "4.4444rem",
              ...
              <div className="grim-mono" style={{ fontSize: "1.1111rem", letterSpacing: ".16em", color: "var(--grim-ink-4)", textTransform: "uppercase", marginTop: 6 }}>
                    ...
                    {editingNoteId === note.id ? (
                      <>
                        ...
                        <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 8 }}>
                          <button className="grim-btn is-ghost" style={{ padding: "4px 10px", fontSize: "1.2222rem" }}
                            onClick={() => { setEditingNoteId(null); setEditingNoteContent(""); }}>
                            Cancel
                          </button>
                          <button className="grim-btn is-ember" style={{ padding: "4px 10px", fontSize: "1.2222rem" }}
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
                        <div style={{ fontSize: "1.5556rem", color: "var(--grim-ink)", lineHeight: 1.5, flex: 1 }}>
                          {!isLegacyNote(note) && isDM && (
                            <span className="grim-chip is-arcane" style={{ fontSize: "1rem", padding: "1px 6px", marginRight: 8, verticalAlign: "middle" }}>DM</span>
                          )}
                        ...
                        <div style={{ display: "flex", gap: 10, flexShrink: 0, paddingTop: 2, alignItems: "center" }}>
                          ...
                          {canEditNote(note) && (
                            <>
                              <a className="grim-link" style={{ fontSize: "1.2222rem", fontFamily: "var(--font-head)", letterSpacing: ".10em", textTransform: "uppercase", cursor: "pointer" }}
                                onClick={() => { setEditingNoteId(note.id); setEditingNoteContent(note.content); }}>
                                edit
                              </a>
                              <a style={{ fontSize: "1.2222rem", fontFamily: "var(--font-head)", letterSpacing: ".10em", textTransform: "uppercase", cursor: "pointer", color: "var(--grim-blood-2)", textDecoration: "none", borderBottom: "1px dotted var(--grim-blood-2)" }}
          ...
          {(() => {
            ...
            return (
              <div style={{ marginTop: 22 }}>
                ...
                <div className="grim-flavor" style={{ fontSize: "1.5556rem", color: "var(--grim-ink-2)", lineHeight: 1.6, borderColor: "var(--grim-arcane)" }}
              ...
              <div className="grim-stack" style={{ gap: 4 }}>
                {appearances.map(r => (
                  <Link key={r.id ?? r.date} href={`/campaign/recaps/${r.id ?? r.date}`}
                    ...
                    <span style={{ fontFamily: "var(--font-head)", fontSize: "1.4444rem", color: "var(--grim-gold-2)", letterSpacing: ".02em" }}>{r.title}</span>
                    <span className="grim-mono" style={{ fontSize: "1rem", color: "var(--grim-ink-4)", letterSpacing: ".12em", flexShrink: 0 }}>{r.date}</span>
                  </Link>
                ))}
              </div>
            </div>
          ...
          {userId && (
            <div style={{ marginTop: 18 }}>
              {editingNote ? (
                ...
                <a className="grim-link" style={{ display: "inline-block", fontFamily: "var(--font-head)", fontSize: "1.3333rem", letterSpacing: ".14em", textTransform: "uppercase", cursor: "pointer" }}
