    ...
    return (
      <div style={{ padding: "36px 56px" }}>
        ...
        <div style={{ textAlign: "center", padding: "60px 24px", color: "var(--grim-ink-4)" }}>
          <div style={{ fontFamily: "var(--font-display)", fontSize: "2.25rem", color: "var(--grim-ink-3)", marginBottom: 8 }}>~ session not found ~</div>
        </div>
      </div>
    );
  }
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16, marginBottom: 14, paddingBottom: 14, borderBottom: "1px solid var(--grim-line)" }}>
          <div style={{ minWidth: 0, flex: 1 }}>
            ...
            {editing ? (
              <input
                ...
                style={{ marginTop: 4, background: "var(--grim-bg-4)", border: "1px solid var(--grim-line-2)", color: "var(--grim-ink)", fontFamily: "var(--font-display)", fontSize: "1.75rem", padding: "6px 12px", outline: "none", width: "100%" }}
              />
            ) : (
              <h1 style={{ fontFamily: "var(--font-display)", fontSize: "2.5rem", color: "var(--grim-gold)", margin: "4px 0 0", lineHeight: 1.05 }}>
                {recap.title}
              </h1>
            )}
          </div>
          {canEdit(recap) && (
            <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
              {editing ? (
                <>
                  <button onClick={() => { setEditing(false); setEditingRecap({}); setError(null); }} className="grim-btn is-ghost" style={{ padding: "6px 12px", fontSize: "0.6875rem" }}>Cancel</button>
                  <button onClick={handleSaveEdit} className="grim-btn is-ember" style={{ padding: "6px 12px", fontSize: "0.6875rem" }} disabled={isSaving}>
          ...
          <div style={{ marginTop: 20, paddingTop: 16, borderTop: "1px dashed var(--grim-line)" }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
              ...
              {(recap.tagged_locations ?? []).map(id => {
                ...
                return l ? <Link key={id} href={`/campaign/locations/${id}`} className="grim-chip is-arcane" style={{ fontSize: "0.6875rem", textDecoration: "none" }}>{l.name}</Link> : null;
              })}
              {(recap.tagged_quests ?? []).map(id => {
                const qt = availableQuests.find(x => x.id === id);
                return qt ? <Link key={id} href={`/campaign/quests/${id}`} className="grim-chip is-faction" style={{ fontSize: "0.6875rem", textDecoration: "none" }}>{qt.name}</Link> : null;
              })}
              ...
              {(recap.tagged_factions ?? []).map(id => {
                ...
                return f ? <Link key={id} href={`/campaign/factions/${id}`} className="grim-chip" style={{ fontSize: "0.6875rem", textDecoration: "none", background: "oklch(0.50 0.14 285 / 0.18)", border: "1px solid oklch(0.50 0.14 285 / 0.45)", color: "var(--grim-arcane)" }}>⚑ {f.name}</Link> : null;
              })}
