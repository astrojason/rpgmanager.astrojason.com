    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: 0, height: "100%", overflow: "hidden" }}>
      ...
      <div style={{ overflowY: "auto", padding: "36px 40px 80px 56px" }}>
        ...
        <section style={{ display: "flex", gap: 12, marginBottom: 22 }}>
          <div style={{ position: "relative", flex: 1 }}>
            ...
            />
            <span style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", color: "var(--grim-gold-2)", fontSize: "1.125rem", pointerEvents: "none" }}>✦</span>
          </div>
          <button
        ...
        <div className="grim-stack" style={{ gap: 22 }}>
          {filteredRecaps.length === 0 ? (
            <div style={{ textAlign: "center", padding: "60px 24px", color: "var(--grim-ink-4)" }}>
              <div style={{ fontFamily: "var(--font-display)", fontSize: "2.25rem", color: "var(--grim-ink-3)", marginBottom: 8 }}>
                ~ no sessions found ~
              </div>
              <div className="grim-mono" style={{ fontSize: "0.6875rem", letterSpacing: ".18em", textTransform: "uppercase" }}>
                Adjust your search
              </div>
            </div>
          ) : (
                ...
                >
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16, marginBottom: 14, paddingBottom: 14, borderBottom: "1px solid var(--grim-line)" }}>
                    <div style={{ minWidth: 0, flex: 1 }}>
                      <div className="grim-mono" style={{ fontSize: "0.625rem", letterSpacing: ".18em", color: "var(--grim-ember-2)", textTransform: "uppercase" }}>
                      ...
                      {editingRecapId === recap.id ? (
                        ...
                        <h3 style={{ fontFamily: "var(--font-display)", fontSize: "2rem", color: "var(--grim-gold)", margin: "4px 0 0", lineHeight: 1.05 }}>
                          {recap.title}
                        </h3>
                      )}
                    </div>
                    ...
                    <div style={{ marginTop: 20, paddingTop: 16, borderTop: "1px dashed var(--grim-line)" }}>
                      ...
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                        {(recap.tagged_npcs ?? []).map(id => {
                          ...
                          return (
                            <Link key={id} href={`/campaign/npcs/${id}`} className="grim-chip is-ember" style={{ fontSize: "0.6875rem", textDecoration: "none", display: "inline-flex", alignItems: "center", gap: 5 }}>
                              {n.name}
                              {n.hidden && <span style={{ fontSize: "0.5625rem", opacity: 0.75 }}>(hidden)</span>}
                              {n.nameHidden && isAdmin && <span style={{ fontSize: "0.5625rem", opacity: 0.75 }}>(name hidden)</span>}
                            </Link>
                          );
                        })}
                        {(recap.tagged_locations ?? []).map(id => {
                          ...
                          return l ? (
                            <Link key={id} href={`/campaign/locations/${id}`} className="grim-chip is-arcane" style={{ fontSize: "0.6875rem", textDecoration: "none" }}>
                              {l.name}
                            </Link>
                          ) : null;
                        })}
                        {(recap.tagged_quests ?? []).map(id => {
                          ...
                          return qt ? (
                            <Link key={id} href={`/campaign/quests/${id}`} className="grim-chip is-faction" style={{ fontSize: "0.6875rem", textDecoration: "none" }}>
                              {qt.name}
                            </Link>
                          ) : null;
                        })}
                        {(recap.tagged_items ?? []).map(id => {
                          ...
                          return (
                            <Link key={id} href={`/campaign/items/${id}`} className="grim-chip" style={{ fontSize: "0.6875rem", textDecoration: "none", display: "inline-flex", alignItems: "center", gap: 5, background: "oklch(0.55 0.090 145 / 0.18)", border: "1px solid oklch(0.55 0.090 145 / 0.45)", color: "var(--grim-moss)" }}>
                              ⚔ {it.name}
                              {it.hidden && <span style={{ fontSize: "0.5625rem", opacity: 0.75 }}>(hidden)</span>}
                            </Link>
                          );
                        })}
                        {(recap.tagged_factions ?? []).map(id => {
                          ...
                          return f ? (
                            <Link key={id} href={`/campaign/factions/${id}`} className="grim-chip" style={{ fontSize: "0.6875rem", textDecoration: "none", background: "oklch(0.50 0.14 285 / 0.18)", border: "1px solid oklch(0.50 0.14 285 / 0.45)", color: "var(--grim-arcane)" }}>
                              ⚑ {f.name}
                            </Link>
                          ) : null;
                        })}
                        {(recap.tagged_deities ?? []).map(id => {
                          ...
                          if (!d) return null;
                          return (
                            <Link key={id} href={`/campaign/deities/${id}`} className="grim-chip" style={{ fontSize: "0.6875rem", textDecoration: "none", display: "inline-flex", alignItems: "center", gap: 5, background: "oklch(0.55 0.10 60 / 0.18)", border: "1px solid oklch(0.55 0.10 60 / 0.45)", color: "var(--grim-gold)" }}>
                              ✦ {d.name}
                              {d.hidden && <span style={{ fontSize: "0.5625rem", opacity: 0.75 }}>(hidden)</span>}
                            </Link>
                          );
                        })}
                      </div>
      ...
      <aside style={{ borderLeft: "1px solid var(--grim-line)", overflowY: "auto", padding: "22px 0" }}>
        <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", marginBottom: 14, padding: "0 22px" }}>
          ...
          <span className="grim-mono" style={{ fontSize: "0.625rem", color: "var(--grim-ink-4)", letterSpacing: ".14em" }}>
        ...
        <div className="grim-stack" style={{ gap: 4 }}>
          {filteredRecaps.map((recap) => {
            ...
            return (
              ...
              >
                <div className="grim-mono" style={{ fontSize: "0.5625rem", letterSpacing: ".14em", color: "var(--grim-ink-4)", textTransform: "uppercase" }}>
                  {recap.date}{sessionNo ? ` · s${sessionNo}` : ""}
                </div>
                <div style={{ fontFamily: "var(--font-head)", fontSize: "0.8125rem", letterSpacing: ".02em", color: isActive ? "var(--grim-ember-2)" : "var(--grim-ink-2)", lineHeight: 1.25, marginTop: 2 }}>
                  {recap.title}
                </div>
              </div>
            );
