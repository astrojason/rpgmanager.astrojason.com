    ...
    <div style={{ padding: "36px 56px 80px", overflowY: "auto", height: "100%" }}>
      ...
      <header style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 24, marginBottom: 28 }}>
        ...
        <div style={{ textAlign: "right", paddingBottom: 6 }}>
          ...
          <div style={{ fontFamily: "var(--font-display)", fontSize: "2.8889rem", color: "var(--grim-gold)", lineHeight: 1 }}>
            The Hellhound Vigil
          </div>
          <div className="grim-mono" style={{ fontSize: "1.2222rem", color: "var(--grim-ink-3)", letterSpacing: ".18em", marginTop: 4 }}>
          ...
          <div className="grim-parchment" style={{ borderRadius: 0, margin: 0, minHeight: 260, padding: "32px 40px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
              <div>
                ...
                <div style={{ fontFamily: "var(--font-display)", fontSize: "4.6667rem", color: "var(--grim-blood)", lineHeight: 1, marginTop: 4 }}>
                  A Summoning
                </div>
              </div>
            ...
            <div style={{ fontFamily: "var(--font-body)", fontSize: "1.7778rem", color: "oklch(0.28 0.03 50)", lineHeight: 1.6, marginTop: 12 }}>
              {sessionDateStr ? (
            ...
            <div style={{ display: "flex", gap: 28, marginTop: 22, paddingTop: 16, borderTop: "1px dashed oklch(0.55 0.08 50 / 0.5)" }}>
              <div>
                ...
                <div style={{ fontFamily: "var(--font-display)", fontSize: "2rem", color: "oklch(0.25 0.03 40)", lineHeight: 1.2, marginTop: 2 }}>
                  {currentGameDate ?? "—"}
                </div>
              </div>
              {sessionData?.location && (
          ...
          <div style={{ padding: "28px 32px", display: "flex", flexDirection: "column", gap: 16, background: "linear-gradient(180deg, oklch(0.16 0.035 290), oklch(0.12 0.030 295))" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                ...
                <span className="grim-mono" style={{ fontSize: "1.1111rem", letterSpacing: ".18em", color: "var(--grim-ember-2)", textTransform: "uppercase" }}>active</span>
              </div>
            </div>

            <div style={{ display: "flex", gap: 14, alignItems: "baseline" }}>
              <div style={{ fontFamily: "var(--font-display)", fontSize: "8.8889rem", color: "var(--grim-ember-2)", lineHeight: 0.85, textShadow: "0 0 30px oklch(0.72 0.165 48 / 0.5)" }}>
                {daysUntil !== null && !sessionData?.isSkipped ? daysUntil : "—"}
              </div>
              <div>
                <div style={{ fontFamily: "var(--font-head)", fontSize: "2.2222rem", color: "var(--grim-gold)", letterSpacing: ".06em" }}>
                  {sessionData?.isSkipped ? "skipped" : daysUntil === 0 ? "today!" : daysUntil === 1 ? "day hence" : "days hence"}
                </div>
                <div className="grim-mono" style={{ fontSize: "1.2222rem", color: "var(--grim-ink-3)", letterSpacing: ".16em", marginTop: 2 }}>
            ...
            {latestRecap && (
              <div style={{ background: "oklch(0.12 0.025 290)", border: "1px solid var(--grim-line)", padding: "14px 16px", borderRadius: 1 }}>
                ...
                <div style={{ fontFamily: "var(--font-body)", fontSize: "1.4444rem", color: "var(--grim-ink)", lineHeight: 1.55, fontStyle: "italic", marginBottom: 8 }}>
                  {toPlainText(latestRecap.recap).slice(0, 220).trimEnd()}…
                </div>
                <Link href="/campaign/recaps" className="grim-link" style={{ fontSize: "1.2222rem", fontFamily: "var(--font-head)", letterSpacing: ".12em", textTransform: "uppercase" }}>
                  {latestRecap.title} ›
                </Link>
              </div>
            )}
          ...
          {QUICK_LINKS.map((s, i) => (
            <a key={i} href={s.href} target={s.href.startsWith("http") ? "_blank" : undefined} rel={s.href.startsWith("http") ? "noopener noreferrer" : undefined}
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontFamily: "var(--font-head)", fontSize: "1.5556rem", letterSpacing: ".10em", textTransform: "uppercase", color: "var(--grim-ink)" }}>{s.title}</div>
                <div className="grim-mono" style={{ fontSize: "1.1111rem", letterSpacing: ".14em", color: "var(--grim-ink-3)", textTransform: "uppercase", marginTop: 2 }}>{s.sub}</div>
              </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.1fr 1.1fr 0.8fr", gap: 18, marginBottom: 28 }}>
        ...
        <section className="grim-tome">
          ...
          <div className="grim-stack" style={{ gap: 10 }}>
                  ...
                  <div style={{ display: "flex", gap: 12, alignItems: "center", padding: "6px 0", borderBottom: i < recentNPCs.length - 1 ? "1px dashed var(--grim-line)" : "none", paddingBottom: i < recentNPCs.length - 1 ? 10 : 0 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "baseline", gap: 6 }}>
                        <div style={{ fontFamily: "var(--font-head)", fontSize: "1.5556rem", color: "var(--grim-ink)", letterSpacing: ".02em", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{name}</div>
                        {nameIsHidden && (isAdmin || isDM) && (
                          <span className="grim-chip" style={{ fontSize: "1rem", padding: "1px 6px", flexShrink: 0, background: "oklch(0.25 0.10 78 / 0.85)", color: "var(--grim-gold-2)", border: "1px solid var(--grim-gold-2)" }}>name hidden</span>
                        )}
                        {npc.pronunciation && !nameIsHidden && <div style={{ fontFamily: "var(--font-body)", fontSize: "1.2222rem", color: "var(--grim-ink-4)", fontStyle: "italic", flexShrink: 0 }}>({npc.pronunciation})</div>}
                      </div>
                      <div className="grim-mono" style={{ fontSize: "1.1111rem", letterSpacing: ".12em", color: "var(--grim-ink-3)", textTransform: "uppercase", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                        {[npc.race, npc.location].filter(Boolean).join(" · ")}
                      </div>
                    </div>
                    <span className={statusChipClass(npc.status)} style={{ flexShrink: 0, fontSize: "1.1111rem" }}>
            ...
            <Link href="/campaign/npcs" className="grim-link" style={{ fontFamily: "var(--font-head)", fontSize: "1.3333rem", letterSpacing: ".14em", textTransform: "uppercase", alignSelf: "flex-start", marginTop: 4 }}>
        ...
        <section className="grim-tome">
          ...
          <div className="grim-stack" style={{ gap: 14 }}>
            {activeQuests.length === 0 ? (
              <p style={{ fontFamily: "var(--font-body)", fontSize: "1.5556rem", color: "var(--grim-ink-4)", fontStyle: "italic", margin: 0 }}>
              ...
              return (
                <Link key={q.id} href={`/campaign/quests/${q.id}`} style={{ display: "flex", gap: 12, alignItems: "flex-start", paddingBottom: 12, borderBottom: i < activeQuests.length - 1 ? "1px dashed var(--grim-line)" : "none", textDecoration: "none" }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontFamily: "var(--font-head)", fontSize: "1.5556rem", letterSpacing: ".02em", color: "var(--grim-ink)" }}>{q.name}</div>
                    <div className="grim-mono" style={{ fontSize: "1.1111rem", letterSpacing: ".14em", color: "var(--grim-ink-4)", textTransform: "uppercase", marginTop: 2 }}>{questMeta(q.status)}</div>
                    {desc && <div style={{ fontSize: "1.4444rem", color: "var(--grim-ink-2)", fontStyle: "italic", marginTop: 6, lineHeight: 1.45 }}>{desc}</div>}
                  </div>
                </Link>
              );
            })}
            <Link href="/campaign/quests" className="grim-link" style={{ fontFamily: "var(--font-head)", fontSize: "1.3333rem", letterSpacing: ".14em", textTransform: "uppercase", alignSelf: "flex-start" }}>
              Unfurl all threads ›
            </Link>
          </div>
        </section>
        ...
        <section className="grim-tome">
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(10, 1fr)", gap: 3, marginBottom: 14 }}>
            {["Adon","Selū","Rili","Tel'","Pyrt","Neld","Vian","Illu","Bari","Anar"].map((d, i) => (
              <div key={i} className="grim-mono" style={{ fontSize: "1rem", letterSpacing: ".04em", color: "var(--grim-ink-4)", textAlign: "center", textTransform: "uppercase", paddingBottom: 4, borderBottom: "1px solid var(--grim-line)" }}>{d}</div>
            ))}
            {Array.from({ length: 40 }).map((_, i) => {
              ...
              return (
                <div key={i} style={{
                  ...
                  fontFamily: "var(--font-display)", fontSize: "1.2222rem",
          ...
          <div className="grim-stack" style={{ gap: 6, fontSize: "1.4444rem" }}>
            <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
              <span className="grim-mono" style={{ fontSize: "1.1111rem", color: "var(--grim-ember-2)", letterSpacing: ".12em" }}>36 ▸</span>
              <span style={{ color: "var(--grim-ink)" }}>The Hellhound Vigil <span className="grim-dim">— today</span></span>
            </div>
            <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
              <span className="grim-mono" style={{ fontSize: "1.1111rem", color: "var(--grim-gold)", letterSpacing: ".12em" }}>40 ▸</span>
              <span style={{ color: "var(--grim-ink-2)" }}>Stormharbor harvest fair</span>
            </div>
            <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
              <span className="grim-mono" style={{ fontSize: "1.1111rem", color: "var(--grim-arcane)", letterSpacing: ".12em" }}>55 ▸</span>
      ...
      <section className="grim-parchment" style={{ marginTop: 6 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 14 }}>
          <div>
            <div className="grim-mono" style={{ fontSize: "1.1111rem", letterSpacing: ".22em", color: "oklch(0.40 0.08 30)", textTransform: "uppercase" }}>Of the House</div>
            <div style={{ fontFamily: "var(--font-display)", fontSize: "4rem", color: "oklch(0.25 0.05 30)", lineHeight: 1 }}>Rules of the Table</div>
          </div>
          <span className="grim-mono" style={{ fontSize: "1.1111rem", color: "oklch(0.45 0.05 40)", letterSpacing: ".18em", textTransform: "uppercase" }}>Three Edicts</span>
        </div>

        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 24, color: "oklch(0.25 0.03 40)" }}>
          {HOUSE_RULES.map((rule, i) => (
            <div key={i}>
              <div style={{ fontFamily: "var(--font-display)", fontSize: "2.4444rem", color: "var(--grim-blood)", marginBottom: 6 }}>
                {rule.roman}. {rule.label}
              </div>
              <p style={{ fontSize: "1.5556rem", lineHeight: 1.55, margin: 0 }}>{rule.body}</p>
            </div>
          ))}
        </div>
      </section>
