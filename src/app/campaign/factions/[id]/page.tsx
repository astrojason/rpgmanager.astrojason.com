    ...
    return (
      <div style={{ padding: "36px 56px" }}>
        ...
        <div style={{ textAlign: "center", padding: "60px 24px", color: "var(--grim-ink-4)" }}>
          <div style={{ fontFamily: "var(--font-display)", fontSize: "3rem", color: "var(--grim-ink-3)", marginBottom: 8 }}>~ banner not found ~</div>
        </div>
      </div>
    );
  }
    ...
    <div style={{ padding: "36px 56px 80px", overflowY: "auto", height: "100%" }}>
      ...
      <section className="grim-tome is-bordered" style={{ padding: 0, overflow: "hidden", marginBottom: 24 }}>
        {factionImage ? (
          <div style={{ position: "relative", height: 220 }}>
            ...
            <button className="grim-btn is-ghost" style={{ position: "absolute", top: 10, right: 10, zIndex: 10, fontSize: "1.1667rem", padding: "4px 10px" }}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 24 }}>
        ...
        <section className="grim-tome">
          ...
          <div className="grim-flavor" style={{ fontSize: "1.1667rem", color: "var(--grim-ink-2)", marginBottom: 14, lineHeight: 1.6 }}
          ...
          {faction.background && (
            <>
              ...
              <div style={{ fontSize: "1.1667rem", color: "var(--grim-ink-2)", lineHeight: 1.6 }}
            ...
            {faction.relationships.map((rel, i) => (
              <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16, paddingBottom: 10, borderBottom: i < faction.relationships!.length - 1 ? "1px dotted var(--grim-line)" : "none" }}>
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontFamily: "var(--font-head)", fontSize: "1.1667rem", color: "var(--grim-ink)", marginBottom: 4 }}>{rel.faction}</div>
                  {rel.description && <div style={{ fontSize: "1.0833rem", color: "var(--grim-ink-3)", lineHeight: 1.5 }}>{rel.description}</div>}
                </div>
            ...
            {recaps.map(r => (
              <Link key={r.id ?? r.date} href={`/campaign/recaps/${r.id ?? r.date}`} style={{ textDecoration: "none" }}>
                <div style={{ display: "flex", gap: 10, alignItems: "baseline" }}>
                  <span className="grim-mono" style={{ fontSize: "0.75rem", letterSpacing: ".12em", color: "var(--grim-ember-2)", flexShrink: 0 }}>SESSION</span>
                  <span style={{ fontFamily: "var(--font-head)", fontSize: "1.0833rem", color: "var(--grim-ink)", letterSpacing: ".02em" }}>{r.title}</span>
                  <span className="grim-mono" style={{ fontSize: "0.75rem", color: "var(--grim-ink-4)", marginLeft: "auto" }}>{r.date}</span>
                </div>
              </Link>
            ))}
            {quests.map(q => (
              <Link key={q.id} href={`/campaign/quests/${q.id}`} style={{ textDecoration: "none" }}>
                <div style={{ display: "flex", gap: 10, alignItems: "baseline" }}>
                  <span className="grim-mono" style={{ fontSize: "0.75rem", letterSpacing: ".12em", color: "var(--grim-gold)", flexShrink: 0 }}>QUEST</span>
                  <span style={{ fontFamily: "var(--font-head)", fontSize: "1.0833rem", color: "var(--grim-ink)", letterSpacing: ".02em" }}>{q.name}</span>
                  <span className="grim-chip" style={{ fontSize: "0.75rem", marginLeft: "auto" }}>{q.status}</span>
                </div>
              </Link>
            ))}
          </div>
...
function HeraldCrest({ type }: { type: string }) {
  return (
    <div style={{
      ...
      fontFamily: "var(--font-display)", fontSize: "3.3333rem", color: "var(--grim-gold)",
...
function BannerText({ faction, totalMembers }: { faction: Faction; totalMembers: number }) {
  return (
    <div>
      <div className="grim-mono" style={{ fontSize: "0.8333rem", letterSpacing: ".24em", color: "var(--grim-gold-2)", textTransform: "uppercase" }}>{faction.type}</div>
      <h1 style={{ fontFamily: "var(--font-display)", fontSize: "4.3334rem", color: "var(--grim-gold)", margin: "2px 0 4px", lineHeight: 0.9, textShadow: "0 0 32px oklch(0 0 0 / 0.4)" }}>
        {faction.name}
      </h1>
      <div style={{ fontFamily: "var(--font-body)", fontSize: "1.0833rem", color: "var(--grim-ink-3)", marginBottom: 8 }}>({faction.pronunciation})</div>
      ...
      >
        ...
        <div style={{ minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <div style={{ fontFamily: "var(--font-head)", fontSize: "1.1667rem", color: deceased ? "var(--grim-ink-3)" : "var(--grim-ink)", letterSpacing: ".02em", textDecoration: deceased ? "line-through" : "none", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
              {name}
            </div>
            {hidden && <span className="grim-chip is-blood" style={{ fontSize: "0.75rem", padding: "1px 6px", flexShrink: 0 }}>hidden</span>}
            {nameHidden && <span className="grim-chip" style={{ fontSize: "0.75rem", padding: "1px 6px", flexShrink: 0, background: "oklch(0.25 0.10 78 / 0.85)", color: "var(--grim-gold-2)", border: "1px solid var(--grim-gold-2)" }}>name hidden</span>}
          </div>
          <div className="grim-mono" style={{ fontSize: "0.8333rem", letterSpacing: ".10em", color: "var(--grim-ink-3)", textTransform: "uppercase", marginTop: 2, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
            {sub}
          </div>
        </div>
      </div>
