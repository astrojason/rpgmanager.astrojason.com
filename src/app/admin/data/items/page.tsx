    ...
    <div style={{ padding: "36px 48px 80px" }}>
      <header style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 24, marginBottom: 28 }}>
        <div>
          ...
          <h1 className="grim-page-title" style={{ fontSize: "6.4444rem" }}>The Armoury &amp; Reliquary</h1>
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontFamily: "var(--font-head)", fontSize: "1.5556rem", color: selectedItem?.id === item.id ? "var(--grim-ember-2)" : "var(--grim-ink-2)", lineHeight: 1.2, display: "flex", alignItems: "center", gap: 6 }}>
                        <span>{item.name}</span>
                        {item.hidden && <span style={{ fontSize: "1.1111rem", color: "var(--grim-blood-2)", fontFamily: "var(--font-mono)", letterSpacing: ".10em" }}>HIDDEN</span>}
                      </div>
                      {item.category && (
                    ...
                    <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                      <a className="grim-link" style={{ fontSize: "1.2222rem", fontFamily: "var(--font-head)", letterSpacing: ".10em", textTransform: "uppercase", cursor: "pointer" }} onClick={e => { e.stopPropagation(); handleEdit(item); }}>edit</a>
                      <a style={{ fontSize: "1.2222rem", fontFamily: "var(--font-head)", letterSpacing: ".10em", textTransform: "uppercase", cursor: "pointer", color: "var(--grim-blood-2)", textDecoration: "none", borderBottom: "1px dotted var(--grim-blood-2)" }} onClick={e => { e.stopPropagation(); handleDelete(item); }}>del</a>
                    </div>
                  </div>
                </div>
              ))}
          ...
          {(isCreating || isEditing) ? (
            <div className="grim-tome">
              ...
              <form onSubmit={e => { e.preventDefault(); handleSave(); }}>
                ...
                <label style={{ display: "inline-flex", alignItems: "center", gap: 8, cursor: "pointer", fontFamily: "var(--font-body)", fontSize: "1.5556rem", color: "var(--grim-ink-2)", marginBottom: 20 }}>
            ...
            <div className="grim-tome">
              <div className="grim-tome-head">
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontFamily: "var(--font-display)", fontSize: "4rem", color: "var(--grim-gold)", lineHeight: 1, marginBottom: 6 }}>{selectedItem.name}</div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
                <div className="grim-stack" style={{ gap: 8 }}>
                  {selectedItem.pronunciation && <div><span className="grim-label">Pronunciation </span><span style={{ fontFamily: "var(--font-mono)", fontSize: "1.4444rem", color: "var(--grim-ink-3)" }}>{selectedItem.pronunciation}</span></div>}
                  {selectedItem.type_tag && <div><span className="grim-label">Type </span><span style={{ fontFamily: "var(--font-body)", fontSize: "1.5556rem", color: "var(--grim-ink-2)", fontStyle: "italic" }}>{selectedItem.type_tag}</span></div>}
                  {(selectedItem.tagged_npcs?.length ?? 0) > 0 && <div><span className="grim-label">NPCs </span><span style={{ fontFamily: "var(--font-body)", fontSize: "1.4444rem", color: "var(--grim-ink-2)" }}>{selectedItem.tagged_npcs?.map(id => availableNpcs.find(n => n.id === id)?.name || id).join(", ")}</span></div>}
                  {(selectedItem.tagged_pcs?.length ?? 0) > 0 && <div><span className="grim-label">PCs </span><span style={{ fontFamily: "var(--font-body)", fontSize: "1.4444rem", color: "var(--grim-ink-2)" }}>{selectedItem.tagged_pcs?.map(id => availablePcs.find(p => p.id === id)?.name || id).join(", ")}</span></div>}
                  {(selectedItem.tagged_locations?.length ?? 0) > 0 && <div><span className="grim-label">Locations </span><span style={{ fontFamily: "var(--font-body)", fontSize: "1.4444rem", color: "var(--grim-ink-2)" }}>{selectedItem.tagged_locations?.map(id => availableLocations.find(l => l.id === id)?.name || id).join(", ")}</span></div>}
                </div>
              </div>
            ...
            <div className="grim-tome" style={{ textAlign: "center", padding: "60px 24px" }}>
              ...
              <div style={{ fontFamily: "var(--font-head)", fontSize: "1.7778rem", letterSpacing: ".08em", textTransform: "uppercase", color: "var(--grim-ink-2)", marginBottom: 8 }}>No relic selected</div>
              <div style={{ color: "var(--grim-ink-4)", fontSize: "1.5556rem" }}>Select a relic from the register, or catalogue a new one.</div>
            </div>
          )}
        </div>
      </div>
