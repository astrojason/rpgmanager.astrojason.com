    ...
    <Link href={href} style={{ textDecoration: "none", color: "inherit", display: "block" }}>
      <div className="grim-tome" style={{ padding: "20px 22px", cursor: "pointer", display: "flex", gap: 16, alignItems: "center" }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontFamily: "var(--font-head)", fontSize: "1.7778rem", letterSpacing: ".06em", textTransform: "uppercase", color: "var(--grim-ink)" }}>{title}</div>
          <div className="grim-mono" style={{ fontSize: "1.1111rem", letterSpacing: ".12em", color: "var(--grim-ink-3)", textTransform: "uppercase", marginTop: 3 }}>{sub}</div>
        </div>
        {count != null && (
          <div style={{ textAlign: "right", flexShrink: 0 }}>
            <div style={{ fontFamily: "var(--font-display)", fontSize: "3.1111rem", color: "var(--grim-gold)", lineHeight: 1 }}>{count}</div>
            <div className="grim-mono" style={{ fontSize: "1rem", letterSpacing: ".16em", color: "var(--grim-ink-4)", textTransform: "uppercase", marginTop: 2 }}>entries</div>
          </div>
        )}
        <span style={{ color: "var(--grim-ink-4)", fontFamily: "var(--font-display)", fontSize: "2.2222rem", marginLeft: count != null ? 8 : 0 }}>›</span>
      </div>
    </Link>
  );
}
    ...
    <div style={{ padding: "36px 48px 80px" }}>
      ...
      <header style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 24, marginBottom: 28 }}>
        <div>
          ...
          <h1 className="grim-page-title" style={{ fontSize: "6.4444rem" }}>The Scriptorium</h1>
        ...
        {user && (
          <div style={{ textAlign: "right", paddingBottom: 6, flexShrink: 0 }}>
            ...
            <div style={{ fontFamily: "var(--font-display)", fontSize: "2.4444rem", color: "var(--grim-gold)", lineHeight: 1 }}>The Master</div>
            <div style={{ display: "flex", gap: 6, justifyContent: "flex-end", marginTop: 6, alignItems: "center" }}>
              ...
              <span className="grim-mono" style={{ fontSize: "1.1111rem", color: "var(--grim-ink-3)", letterSpacing: ".14em" }}>{user.email}</span>
            </div>
          </div>
        )}
      </header>
          ...
          {([
            ...
            <div key={i} style={{ padding: "22px 26px", borderLeft: i > 0 ? "1px solid var(--grim-line)" : "none", background: "linear-gradient(180deg, oklch(0.17 0.035 285), oklch(0.135 0.030 290))" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                <span style={{ fontFamily: "var(--font-display)", fontSize: "2rem", color: TINT_BORDER[s.tint] }}>{s.glyph}</span>
                <span className="grim-mono" style={{ fontSize: "1rem", letterSpacing: ".18em", color: "var(--grim-ink-4)", textTransform: "uppercase" }}>{s.l}</span>
              </div>
              <div style={{ fontFamily: "var(--font-display)", fontSize: "5.3333rem", color: "var(--grim-gold)", lineHeight: 0.9, textShadow: "0 0 28px oklch(0.72 0.165 48 / 0.15)" }}>
                {s.n ?? <span style={{ fontSize: "2.6667rem", color: "var(--grim-ink-4)" }}>—</span>}
              </div>
            </div>
          ))}
        </div>
      ...
      <div style={{ textAlign: "center", padding: "4px 0 10px", color: "var(--grim-ink-4)", fontFamily: "var(--font-display)", fontSize: "3.1111rem", letterSpacing: ".10em" }}>❦</div>
